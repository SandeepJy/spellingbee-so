    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ...
                TopSpellersCard(players: manager.topSpellers, isLoading: manager.isLoadingTop)
            }
            .padding()
        }
...
struct MyRankCard: View {
            ...
            if let rank = rank {
                HStack(alignment: .lastTextBaseline, spacing: 4) {
                    ...
                    VStack(alignment: .trailing, spacing: 4) {
                        Text(String(format: "%.1f%%", rank.accuracy))
                ...
                if rank.totalPlayers > 1 {
                    VStack(spacing: 4) {
                        ...
                        .frame(height: 8)
                        HStack {
                            Text("Top \(topPercentLabel(rank.rank, rank.totalPlayers))")
                                .font(.caption).foregroundColor(.secondary)
                            Spacer()
                ...
                if rank.totalWordsSpelled > 0 {
                    HStack {
                        Image(systemName: "text.book.closed.fill")
                            .foregroundColor(.orange)
                        Text("\(rank.totalWordsSpelled) words attempted")
                            .font(.caption).foregroundColor(.secondary)
                    }
                }
            } else if !isLoading {
                ...
                RoundedRectangle(cornerRadius: 8)
                    .fill(Color(.systemGray5)).frame(height: 60)
    ...
    private func topPercentLabel(_ rank: Int, _ total: Int) -> String {
        guard total > 1 else { return "1%" }
        return "\(Int(ceil(Double(rank) / Double(total) * 100)))%"
    }
}
            ...
            if isLoading && players.isEmpty {
                VStack(spacing: 8) {
                    ForEach(0..<5, id: \.self) { _ in
                        RoundedRectangle(cornerRadius: 8)
                            .fill(Color(.systemGray5)).frame(height: 52)
                            .redacted(reason: .placeholder)
                    }
                }
            } else if players.isEmpty {
                VStack(spacing: 8) {
                    Image(systemName: "person.3").font(.system(size: 40)).foregroundColor(.secondary)
                    Text("No ranked players yet").font(.subheadline).foregroundColor(.secondary)
                }
...
struct TopSpellerRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            ZStack {
                Circle()
                    .fill(player.isCurrentUser ? Color.blue.opacity(0.15) : rankColor.opacity(player.rank <= 3 ? 0.2 : 0.08))
                    .frame(width: 36, height: 36)
                if player.rank <= 3 {
                    Image(systemName: player.rank == 1 ? "crown.fill" : "medal.fill")
                        .font(.system(size: 16)).foregroundColor(rankColor)
                } else {
                    Text("\(player.rank)")
                        .font(.system(size: 14, weight: .bold, design: .rounded))
                        .foregroundColor(player.isCurrentUser ? .blue : .secondary)
                }
            }
        ...
        .background(RoundedRectangle(cornerRadius: 12).fill(
            player.isCurrentUser ? Color.blue.opacity(0.05) : Color(.systemBackground)
        ))
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .stroke(player.isCurrentUser ? Color.blue.opacity(0.3) : Color.clear, lineWidth: 1)
        )
    }
}
